...
exports.createClient = function(options){
  ...
  _me.unRegisterService = function(id,callback){
    var path = '/'+CONSTANTS.SERVICE_ROOT+'/'+id.name+'/'+id.version+'/'+os.hostname()+process.pid;
    zkTools.del(path,function(err){
      if(err){
        callback(err);
      }else{
  ...
  _me.subscribe = function(name,filter,callback){
    ...
    loadService(key,name,filter,service,function(err2){
      if(err2 === 'connect err'){
        ...
        service.setList(key,[cache[key]]);
        callback('');

        var id;
        id = setInterval(function(){
          var preAvail = service.getServiceAll();
          loadService(key,name,filter,service,function(err3){
            if(!err3){
